...
- **Dependency vulnerabilities** in `package.json`'s real dependency tree (`apify` and its dev dependencies) are a real, ongoing concern — tracked via Dependabot (`.github/dependabot.yml`) and GitHub's own dependency/secret scanning, both enabled on this repository.
