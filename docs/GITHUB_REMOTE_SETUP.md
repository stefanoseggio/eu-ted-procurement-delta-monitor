...
```bash
cd repo/ted-procurement   # or whichever of the 4 actor repos you're setting up
APIFY_TOKEN=apify_api_xxxxxxxx ./docs/setup_github_remote.sh eu-ted-procurement-delta-monitor
```
...
```bash
cd repo/ted-procurement
gh repo create eu-ted-procurement-delta-monitor --private --source=. --remote=origin
...
Repeat this for each of the other 3 repos (`repo/uk-modern-slavery/`, `repo/sovereign-debt/` and `repo/uae-corporate-registry/`) with a distinct repository name each — do not push more than one actor's
